...
// 不知道干什么的一个宏, 尼玛...
#define OIDDEF(tag, oidValue)                              \
    {                                                      \
        tag, { oidValue, sizeof(oidValue), 0, 1, 0, 0, 0 } \
    }
...
ITCAST_UINT32 BaseASN1::DER_ItAsn1_Low_Count_LengthOfSize(ITCAST_UINT32 iLength)
{
    if (iLength <= 0x7F)
        ...
        return (2);
    else if (iLength < 0xFFFF)
        return (3);
    else if (iLength <= 0xFFFFFF)
        return (4);
    else if (iLength <= 0xFFFFFFFF)
        return (5);
    else
    {
...
ITCAST_INT BaseASN1::DER_ItAsn1_Low_WriteTagAndLength(ITCAST_ANYBUF *pAnyIn, ITCAST_UINT8 cTag, ITCAST_ANYBUF **ppAnyOut, ITCAST_UINT8 **ppUint8Value)
{
    ...
    else if (cTag == ITCAST_DER_ID_INTEGER)
